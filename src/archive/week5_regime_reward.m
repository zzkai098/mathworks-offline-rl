%% Offline RL for GBWM — Week 5: Regime-Gated Asymmetric Reward (Plan A)
...
logsRoot        = fullfile("experiments", "logs", "week5_regime_reward");
...
VIX_THR    = 1.0;    % VIX_z above this -> stress
SLOPE_THR  = -1.0;   % T10Y2Y_z below this (curve inversion) -> stress
LAMBDA     = 2.5;    % loss amplification factor during stress
...
fprintf("Worst Sharpe (avg)   : %.2f   <-- v3 baseline: -5.78\n", mean(results.minSharpe));
fprintf("MaxDD P90 (avg)      : %.2f%% <-- v3 baseline: 12.87%%\n", mean(results.p90MaxDD)*100);
...
ylabel('Wealth');
title(sprintf('Week 5 regime-gated reward (lambda=%.1f) — %d paths', LAMBDA, rowCursor));
