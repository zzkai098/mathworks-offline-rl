%% Offline RL for GBWM — Week 8: DDPG with ON-FRONTIER continuous action
% Corrects the portfolio-construction flaw the mentor flagged in Week 7.
%
% Week 7 mapped the continuous action α∈[0,1] to weights by linearly
% interpolating adjacent EFFICIENT-FRONTIER portfolios (15 points):
%   idxF = 1 + α*14 ;  w(α) = lerp(W_frontier(:,floor), W_frontier(:,ceil))
% Problem: expected return is LINEAR in weights but variance is CONVEX, so
% a convex blend of two frontier portfolios has the right return but a
% variance ABOVE the true frontier at that return — an interior,
% mean-variance-SUBOPTIMAL point (except within a single linear frontier
% segment). So "every DDPG portfolio is on the frontier" was false.
%
% Fix: build a DENSE frontier once via estimateFrontierByReturn over a fine
% grid of target returns, and interpolate α over THAT. As the grid
% densifies, the chord residual  t(1-t)(w_i-w_j)'Σ(w_i-w_j)  → 0, so every
% α lands on the efficient frontier to machine precision. A diagnostic
% below reports the residual variance gap to confirm it is negligible.
%
%   α ∈ [0, 1]   (0 = min-return/defensive end, 1 = max-return/aggressive)
%   μ_target(α) via the dense return grid ;  w(α) = interp over W_dense
%
% Because the α→weights map changed, DDPG is RETRAINED here (training and
% evaluation must share the same map).
%
...
horizonPeriods  = 30;
numEpisodes     = 200;
numActions      = 15;          % coarse frontier used only to get return limits
nDense          = 200;         % dense on-frontier grid for the α interpolation

logsRoot        = fullfile("experiments", "logs", "week8_ddpg_onfrontier");
...
W_frontier = estimateFrontier(p, numActions);       % coarse, only for return limits

% --- ON-FRONTIER FIX: dense frontier over a fine grid of target returns ---
% Build once. estimateFrontierByReturn returns true efficient portfolios at
% each target return, so interpolating alpha over W_dense stays on (or
% arbitrarily close to) the frontier — no per-step QP needed.
rmin    = estimatePortReturn(p, W_frontier(:, 1));    % min-return frontier end
rmax    = estimatePortReturn(p, W_frontier(:, end));  % max-return frontier end
muGrid  = linspace(rmin, rmax, nDense);
W_dense = estimateFrontierByReturn(p, muGrid);        % Nassets x nDense

% --- Diagnostic: residual variance gap of the alpha-interpolation vs the
% true frontier at the same target return (the sentence for the mentor). ---
alphaProbe = linspace(0, 1, 51);
maxVarGap  = 0;
for aa = alphaProbe
    wI      = alphaToWeights(aa, W_dense);            % interpolated weights
    muI     = estimatePortReturn(p, wI);             % its expected return
    varI    = estimatePortRisk(p, wI)^2;             % its variance
    wStar   = estimateFrontierByReturn(p, muI);      % true frontier at same mu
    varStar = estimatePortRisk(p, wStar)^2;
    maxVarGap = max(maxVarGap, varI - varStar);      % >= 0 by suboptimality
end
fprintf("On-frontier diagnostic: dense grid = %d pts, max residual variance gap = %.3e\n", ...
    nDense, maxVarGap);
fprintf("  (interpolated portfolios are on the efficient frontier to this tolerance)\n");
...
fprintf("=== Week 8 setup (DDPG, ON-FRONTIER continuous action) ===\n");
...
%% Cross-seed summary
fprintf("\n\n===== Cross-seed summary (n=%d seeds) — Week 8 DDPG on-frontier continuous action =====\n", nSeeds);
...
figure('Name','Hold-out wealth paths (Week 8 DDPG on-frontier continuous action)');
hold on;
...
ylabel('Wealth');
title(sprintf('Week 8: DDPG on-frontier alpha (beta=%.1f/%.1f) — %d paths', ...
