...
% Week 7.0 (binary stress flag in state) showed:
%   - The mechanism WORKS: Q-gap mean 2.24 (target was >0.02, 100x over).
%     The network does use the 7th dim heavily.
%   - Action distribution shifted in the partially-correct direction:
%     defensive #1/#2 selected MORE in stress (+6.8pp, +9.2pp), aggressive
%     #14 selected LESS in stress (-9.4pp). First time we see this signal.
%   - BUT cross-seed instability exploded: Sharpe std 0.23 -> 0.55, with
%     per-seed Q-gap ranging from 0.23 (seed 5000) to 6.67 (seed 4000).
%     Mean Sharpe regressed 0.66 -> 0.54, worst Sharpe -6.37 -> -6.87.
%
% Diagnosis: a binary feature is a sharp decision surface — different seeds
% latch onto qualitatively different uses of it (some heavily depend on it,
% others nearly ignore it), which produces the bimodal across-seed
% behavior. A SMOOTH input signal should provide more stable gradients
% across seeds and dampen this variance.
%
% 7.0b replaces the binary stress flag with a continuous stress score:
%   stressScore(t) = max(0, VIX_z(t)) + max(0, -T10Y2Y_z(t))
%   stressNorm(t)  = clip(stressScore(t) / stressP99, 0, 1)
% where stressP99 is the 99th percentile of stressScore over the train
% window. The normalized score is on [0, 1] so it matches the binary
% version's scale but provides smooth interpolation between regimes.
%
% Same reward gate (binary BETA_HIGH/LOW from 6.1) is kept — only the
% state input changes vs 7.0. This isolates whether smoothness of the
% input signal alone fixes the across-seed instability.
